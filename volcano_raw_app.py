...
def update_combined_norm(fc_cutoff, cohen_cutoff, p_cutoff_log, p_input_val, x_col):
    raw_p_cutoff = float(p_input_val) if dash.callback_context.triggered_id == "combined-norm-p-input" else 10**(-p_cutoff_logPerfect ✅ thanks for clarifying that the second section must use **sheet = `"Cr_Norm"`**.  
Here’s the **full Dash app code** that renders **Raw** and **Cr_Norm** volcano plots side-by-side on the same page, each with its own sliders, dropdowns, and callbacks.  

---

### 🔹 Complete Code (Raw + Cr_Norm)
